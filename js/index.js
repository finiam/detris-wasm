import("../pkg/index.js").then(wasm =>{
  import ("../pkg/index_bg.wasm").then(w => {
    ...
    let CELL_SIZE = Math.min((window.innerHeight - 100) / 20, (window.innerWidth - 100) / 10);
    ...
    window.addEventListener('resize', (e) => {
      CELL_SIZE = Math.min((window.innerHeight - 100) / 20, (window.innerWidth - 100) / 10);
      canvas.width = 10 * (CELL_SIZE + 2) + 2;
      canvas.height = 24 * (CELL_SIZE + 2 ) + 2;
      ...
      for (let i = 0; i < 6; i++) {
        for (let j = 0; j < 6; j++) {
          ctx.fillStyle = `rgb(
              ...
              0)`;
          ctx.fillRect(j * 25 + CELL_SIZE * 4, i * 25 + CELL_SIZE * 4, 25, 25);
        }
      }
